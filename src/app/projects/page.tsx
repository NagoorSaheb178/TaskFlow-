    ...
    <DashboardLayout>
      <div className="max-w-[1440px] mx-auto w-full overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-gutter">
          {loading ? (
            <div className="col-span-full p-8 text-center text-on-surface-variant animate-pulse">Loading projects...</div>
          ) : projects.length === 0 ? (
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-stack-md gap-2">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <h3 className="font-h3 text-h3 text-on-surface truncate pr-2" title={project.name}>{project.name}</h3>
                      </div>
                      {isAdmin && (
                        <button 
                          onClick={() => { setCurrentProject(project); setShowMemberModal(true); }}
                  ...
                  <div className="flex justify-between items-center mt-4 border-t border-outline-variant pt-4">
                    <div className="flex -space-x-2 overflow-hidden max-w-[150px]">
                      {project.members?.slice(0, 4).map((member, i) => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-surface bg-primary-container text-on-primary-container flex items-center justify-center font-label-sm font-bold shrink-0">
          ...
          <div className="fixed inset-0 bg-on-background/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
              ...
              <form onSubmit={handleCreateProject} className="p-6 space-y-4">
                <div>
                  <label className="font-label-md text-on-surface-variant mb-1 block">Project Name *</label>
                  <input
                    type="text" required
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-outline-variant">
                  <button type="button" onClick={() => setShowCreateModal(false)} className="px-5 py-2.5 rounded-xl font-label-md text-on-surface-variant hover:bg-surface-container transition-colors">
                    Cancel
                  </button>
          ...
          <div className="fixed inset-0 bg-on-background/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
              <div className="flex items-center justify-between p-6 border-b border-outline-variant">
                ...
                <button onClick={() => setShowMemberModal(false)} className="p-2 hover:bg-surface-container rounded-full transition-colors">
                  <span className="material-symbols-outlined text-on-surface-variant">close</span>
                </button>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-2 mb-6">
                  ...
                  >
                    ...
                    {users.filter(u => !currentProject.members.find(m => m._id === u._id)).map(user => (
                      <option key={user._id} value={user._id}>{user.name} ({user.email})</option>
                    ))}
                  </select>
                  <button 
                    onClick={handleAddMember}
                    className="bg-primary text-on-primary font-label-md px-4 py-2.5 rounded-xl flex items-center gap-1 active:scale-95 transition-transform"
                  >
                    <span className="material-symbols-outlined text-[18px]">add</span>
                    Add
                  </button>
                ...
                <div className="divide-y divide-outline-variant max-h-60 overflow-y-auto">
                  {currentProject.members.map(member => (
                    <div key={member._id} className="flex items-center justify-between py-3">
                      ...
                      <button 
                        ...
                        className="text-error p-2 hover:bg-error-container/20 rounded-full transition-colors"
                        title="Remove member"
                      >
                  ...
                  {currentProject.members.length === 0 && (
                    <p className="text-center py-4 text-on-surface-variant font-label-md">No members yet.</p>
                  )}
                </div>
              </div>
            </div>
